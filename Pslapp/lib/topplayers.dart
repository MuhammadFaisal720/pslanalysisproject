...
class _TopPlayersState extends State<TopPlayers> {
  ...
  fetchUser() async {
    ...
    var response = await http.get(Uri.https(url,'/top_players'));
    print(response.body);
    if(response.statusCode == 200){
      var items = json.decode(response.body);
      print(items);
      print(items[0]['Rank']);
      print(items[0]['Players']);
      print(items[0]['Score']);
      setState(() {
        users = items;
        isLoading = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("PSL Top Players Prediction"),
      ),
      body: getBody(),
    );
  }
  ...
  Widget getCard(item){
    ...
    var teamname="Team:"+item['Players'].toString();
